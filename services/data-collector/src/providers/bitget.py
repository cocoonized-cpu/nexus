...
Bitget is a Tier 2 exchange with 8-hour funding intervals.
"""
...
class BitgetProvider(ExchangeProvider):
    """Bitget perpetual futures data provider."""

    def __init__(self):
        super().__init__()
        self._client: Optional[ccxt.bitget] = None
    ...
    async def initialize(self) -> None:
        ...
        logger.info(f"Bitget provider initialized", symbols=len(self._symbols))

    async def close(self) -> None:
        """Close the Bitget client."""
        if self._client:
        ...
        try:
            for symbol in self._symbols[:60]:
                try:
                    ...
                    logger.warning(
                        f"Failed to fetch Bitget funding rate",
                        symbol=symbol,
                        error=str(e),
                    )
            ...
            logger.error("Failed to fetch Bitget funding rates", error=str(e))
            self._record_error(str(e))
    ...
    async def get_prices(self) -> list[PriceData]:
        ...
        try:
            ...
            logger.error("Failed to fetch Bitget prices", error=str(e))
            self._record_error(str(e))

        return prices

    async def get_liquidity(self) -> list[LiquidityData]:
        """Fetch order book liquidity from Bitget."""
        if not self._client:
            return []
        ...
        for symbol in top_symbols:
            try:
                ...
                logger.warning(
                    f"Failed to fetch Bitget orderbook", symbol=symbol, error=str(e)
                )
                self._record_error(str(e))
